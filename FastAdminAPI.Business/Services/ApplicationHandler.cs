    ...
    public class ApplicationHandler : IApplicationHandler
    {
            ...
            if (postId != null)
            {
                ...
                if (parentPost != null)
                {
                    ...
                    superior = await _dbContext.Queryable<S08_EmployeePost>()
                        ...
                        .Where((S08, S07) => S08.S06_PostId == parentPost &&
        ...
        private async Task<ApproverInfoModel> GetDepartSuperiorRecursive(List<S05_Department> departList, List<S06_Post> postList, long departId, long originDepartId)
        {
            ...
            if (parentDepartId > 0)
            {
                ...
                if (superPostId > 0)
                {
                    ...
                    superior = await _dbContext.Queryable<S08_EmployeePost>()
                        .LeftJoin<S07_Employee>((S08, S07) => S08.S07_EmployeeId == S07.S07_EmployeeId)
                        .Where((S08, S07) => S08.S06_PostId == superPostId &&
                                             S08.S08_IsValid == (byte)BaseEnums.IsValid.Valid &&
                                             S08.S08_IsMainPost == (byte)BaseEnums.TrueOrFalse.True)
                        .OrderBy((S08, S07) => S08.S08_CreateTime, OrderByType.Desc)
                        .Select((S08, S07) => new ApproverInfoModel
        ...
        private async Task<ApproverInfoModel> GetSuperiorApprover(long operationId, string operationName)
        {
            ...
            var mainPost = await _dbContext.Queryable<S08_EmployeePost>()
                .Where(S08 => S08.S07_EmployeeId == operationId &&
                              S08.S08_IsValid == (byte)BaseEnums.IsValid.Valid &&
                              S08.S08_IsMainPost == (byte)BaseEnums.TrueOrFalse.True)
        ...
        private async Task<List<ApproverInfoModel>> GetDesigneeApprovers(List<long> approverIds)
        {
            if (approverIds?.Count > 0)
            {
                ...
                var approvers = await _dbContext.Queryable<S07_Employee>()
                    .Where(S07 => approverIds.Contains(S07.S07_EmployeeId) &&
        ...
        #region 审批流程类型：指定金额上限审批人
        /// <summary>
        /// 获取指定金额上限审批人
        /// </summary>
        /// <param name="approverIds">审批人Id</param>
        /// <param name="amounts">金额上限区间</param>
        /// <param name="targetAmount">目标金额</param>
        /// <returns></returns>
        /// <exception cref="UserOperationException"></exception>
        private async Task<List<ApproverInfoModel>> GetDesigneeApproversWithAmounts(List<long> approverIds, List<double> amounts, double? targetAmount)
        {
            if (targetAmount == null)
                throw new UserOperationException("目标金额未设置，无法进行申请!");

            //审批人与金额需一一对应
            if (approverIds?.Count > 0 && amounts?.Count > 0 && approverIds.Count == amounts.Count)
            {
                //目标审批人
                List<long> targetApproverIds = new();

                //循环比较 只匹配区间内的一位审批人并返回
                for (int i = 0; i <= amounts.Count - 1; i++)
                {
                    //如果目标金额小于第一金额挡位，直接退出循环
                    if (i == 0 && targetAmount < amounts[0])
                    {
                        break;
                    }
                    //如果目标金额比最后一位金额上限还大，直接返回最后一位审批人
                    if (i == amounts.Count - 1 && targetAmount >= amounts[i])
                    {
                        targetApproverIds.Add(approverIds[i]);
                        break;
                    }
                    //如果目变金额在当前金额上限及后一位金额上限之间，返回当前审批人
                    if (targetAmount >= amounts[i] && targetAmount < amounts[i + 1])
                    {
                        targetApproverIds.Add(approverIds[i]);
                        break;
                    }
                ...
                if (targetApproverIds?.Count > 0)
                {
                    //查询设置的审批人
                    var approvers = await _dbContext.Queryable<S07_Employee>()
                        .Where(S07 => targetApproverIds.Contains(S07.S07_EmployeeId) &&
                                      S07.S07_IsValid == (byte)BaseEnums.IsValid.Valid)
                        .Select(S07 => new
                        {
                            StaffId = S07.S07_EmployeeId,
                    ...
                    //循环赋值
                    targetApproverIds.ForEach(item =>
                    {
                        var approver = approvers?.Where(c => c.StaffId == item).FirstOrDefault();
                        //判断审批人是否存在，不存在使用默认审批人
                        if (approver != null)
                ...
                throw new UserOperationException("获取审批人失败或配置错误!");
        }
        #endregion
        ...
        private async Task<ApproverInfoModel> GetDefaultApprover()
        {
            var defaultApprover = await _dbContext.Queryable<S07_Employee>()
                        .Where(S07 => S07.S07_EmployeeId == DEFAULT_APPROVER &&
                                      S07.S07_IsValid == (byte)BaseEnums.IsValid.Valid)
                        .Select(S07 => new ApproverInfoModel
                        {
                            EmployeeId = S07.S07_EmployeeId,
        ...
        private async Task<List<CarbonCopiesInfoModel>> GetCarbonCopies(S11_CheckProcess process, List<long> extenalCarbonCopiesIds = null)
        {
            ...
            if (carbonCopiesIds?.Count > 0)
            {
                ...
                var ccReceivers = await _dbContext.Queryable<S07_Employee>()
                    .Where(S07 => carbonCopiesIds.Contains(S07.S07_EmployeeId) &&
                                  S07.S07_IsValid == (byte)BaseEnums.IsValid.Valid)
        ...
        private async Task<S11_CheckProcess> GetApproveProcess(long checkProcessType, long employeeId)
        {
            ...
            var processList = await _dbContext.Queryable<S11_CheckProcess>()
                .Where(Z04 => Z04.S99_ApplicationType == checkProcessType && Z04.S11_IsValid == (byte)BaseEnums.IsValid.Valid)
            ...
            if (process == null)
            {
                process = await _dbContext.Queryable<S11_CheckProcess>()
                    .Where(Z04 => Z04.S99_ApplicationType == checkProcessType &&
                                  (Z04.S07_Applicants == null || Z04.S07_Applicants == "") &&
                                  Z04.S11_IsValid == (byte)BaseEnums.IsValid.Valid)
                    .FirstAsync();
                if (process == null)
        ...
        private async Task<ApprovalDataByRedisModel> GetApprovers(SetApplicationModel model, S11_CheckProcess process)
        {
            ...
            #region 上级+指定审批人+金额
            else if (process.S11_ApproveType == (byte)ApplicationEnums.ApproveType.SuperiorAndDesigneeWithAmount)
            {
                if (model.TotalAmount == null)
                    throw new UserOperationException("当前申请[审批类型]错误(该申请不包含金额)，无法进行申请!");
                ...
                //按金额上限获取审批人
                List<long> approverIds = process.S07_Approvers?.Split(",").Select(x => Convert.ToInt64(x)).ToList();
                List<double> amounts = process.S11_Amounts?.Split(",").Select(x => Convert.ToDouble(x)).ToList();
                approverList.AddRange(await GetDesigneeApproversWithAmounts(approverIds, amounts, model.TotalAmount));
