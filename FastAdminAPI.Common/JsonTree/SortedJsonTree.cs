...
namespace FastAdminAPI.Common.JsonTree
{
    ...
    /// 有序Json树结构
    /// </summary>
    public class SortedJsonTree : SortedJsonTree<SortedJsonTree> { }
    /// <summary>
    /// 有序Json树结构
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public class SortedJsonTree<T> where T : SortedJsonTree<T>
    {
        /// <summary>
        /// Id
        /// </summary>
        ...
        private static List<T> CreateTree(List<T> dataSource)
        {
            if (dataSource?.Count > 0)
            {
                ...
                //获取子级
                var children = dataSource.Where(c => c.ParentId != null && c.ParentId != 0).OrderBy(c => c.Priority).ToList();

                parents?.ForEach(item => { item.AddChildren(children); });
        ...
        private static List<T> CreateCustomTree(List<T> dataSource, List<T> parents)
        {
            ...
            {
                parents.OrderBy(c => c.Priority).ToList().ForEach(item =>
                {
        ...
        {
            var childrenByParent = dataSource?.Where(c => c.ParentId == this.ParentId).OrderBy(c => c.Priority).ToList();
